...
<?php
	...
	//page에 값이 설정되어 있는지 확인하는 함수 isset()
	...
	for ($i=$start; $i<$start+$scale && $i < $total_record; $i++)
	{
		...
		if ($row["file_name"]) 
			...
			$file_image = " "; //존재하지 않을 경우 공백 저장
?>
			<li>
				<span class="col1"><?=$number?></span> <!-- 게시글 번호 -->
				<!-- 글 목록의 각 항목을 화면에 출력 -->
				<span class="col2"><a href="board_view.php?num=<?=$num?>&page=<?=$page?>"><?=$subject?></a></span> <!-- 제목 -->

				<span class="col3"><?=$name?></span> <!-- 글쓴이 -->
				<span class="col4"><?=$file_image?></span> <!-- 첨부파일아이콘 -->
